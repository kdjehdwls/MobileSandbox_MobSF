...
def decrypt_dex(encrypted_dex_path, decrypted_dex_path, aes_key, aes_iv):
    ...
    try:
        ...
        # print(f"원본 DEX 파일 삭제 성공: {encrypted_dex_path}")
...
def check_scan_status(mobsf_url, api_key, scan_id):
    """Check the scan status until it's completed."""
    headers = {'Authorization': api_key}
    while True:
        response = requests.get(f'{mobsf_url}/api/v1/scan_status', params={'scan_id': scan_id}, headers=headers)
        if response.status_code == 200:
...
def download_pdf_report(mobsf_url, api_key, scan_id, output_dir):
    ...
    headers = {'Authorization': api_key}
    data = {'hash': scan_id}
    response = requests.post(f'{mobsf_url}/api/v1/download_pdf', data=data, headers=headers, stream=True)
    if response.status_code == 200:
        output_path = os.path.join(output_dir, f'{scan_id}.pdf')
        with open(output_path, 'wb') as f:
            for chunk in response.iter_content(chunk_size=1024):
                if chunk:  # 필터링 된 청크를 파일에 쓰기
                    f.write(chunk)
...
if __name__ == "__main__":
    ...
    if file_hash:
        scan_id = start_scan(mobsf_url, api_key, file_hash)  # 스캔 시작하고 scan_id를 받음
        if scan_id:
            # 스캔 상태 확인
            check_scan_status(mobsf_url, api_key, scan_id)

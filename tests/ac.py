...
import os
import requests
from Cryptodome.Cipher import AES # 패딩은 필요없는듯? cryto나 cryptography로도 해봤으나 같은문제.
import glob
...
def decrypt_dex(encrypted_dex_path, decrypted_dex_path, aes_key_str):
    # 키를 바이트 배열로 변환
...
class MobSF_API:
    ...
    def extract_activities_from_manifest(decompiled_apk_dir):
        """AndroidManifest.xml에서 모든 Activity의 이름을 추출"""
    ...
    def start_activity(package_name, activity_name):
        """ADB를 사용하여 특정 Activity를 시작"""
        try:
            cmd = ['adb', 'shell', 'am', 'start', '-n', f'{package_name}/{activity_name}']
            subprocess.run(cmd, check=True)
            print(f"Activity 시작 성공: {activity_name}")
        except subprocess.CalledProcessError as e:
            print(f"Activity 시작 실패: {e}")

    def perform_dynamic_analysis_on_activities(mobSF, file_hash, package_name, decompiled_apk_dir):
        """모든 Activity에 대해 동적 분석 수행"""
        activities = extract_activities_from_manifest(decompiled_apk_dir)
        for activity in activities:
            start_activity(package_name, activity)
            # 동적 분석 상태를 확인하고 필요한 데이터를 수집
            mobSF.get_dynamic_analysis_status(file_hash)
            # 여기에 추가 분석 로직을 구현할 수 있습니다.
    
    def analyze_network_traffic(mobSF, file_hash):
        """네트워크 트래픽 분석"""
        # 동적 분석 데이터에서 네트워크 트래픽 관련 정보 추출
        dynamic_analysis_data = mobSF.get_dynamic_analysis_status(file_hash)
        network_traffic = dynamic_analysis_data.get('network_traffic', {})
        
        # 여기에서 네트워크 트래픽 데이터를 분석하고 결과를 출력 또는 기록합니다.
        # 예: 의심스러운 호스트로의 연결, 비정상적인 데이터 전송 패턴 등을 탐지합니다.
        suspicious_hosts = ['example.com', 'malicious-site.com']
        for host, traffic_details in network_traffic.items():
            if host in suspicious_hosts:
                print(f"의심스러운 네트워크 활동 탐지: {host}")
                # 추가 분석 또는 경고 로직

    def monitor_file_system_changes(mobSF, file_hash):
        """파일 시스템 변경 모니터링"""
        # 동적 분석 데이터에서 파일 시스템 변경 정보 추출
        dynamic_analysis_data = mobSF.get_dynamic_analysis_status(file_hash)
        file_changes = dynamic_analysis_data.get('file_changes', {})

        # 여기에서 파일 시스템 변경 데이터를 분석하고 결과를 출력 또는 기록합니다.
        # 예: 민감한 디렉토리에 대한 의심스러운 쓰기 작업 감지
        sensitive_directories = ['/data/data/', '/sdcard/']
        for change_type, files in file_changes.items():
            for file_path in files:
                if any(dir in file_path for dir in sensitive_directories):
                    print(f"민감한 디렉토리 변경 탐지: {file_path}")
                    # 추가 분석 또는 경고 로직

    def detect_abnormal_system_calls(mobSF, file_hash):
        """비정상적인 시스템 호출 감지"""
        # 동적 분석 데이터에서 시스템 호출 관련 정보 추출
        dynamic_analysis_data = mobSF.get_dynamic_analysis_status(file_hash)
        system_calls = dynamic_analysis_data.get('system_calls', {})

        # 여기에서 시스템 호출 데이터를 분석하고 결과를 출력 또는 기록합니다.
        # 예: 특정 시스템 호출의 비정상적인 빈도나 순서를 탐지합니다.
        abnormal_calls = ['fork', 'exec', 'system']
        for call, frequency in system_calls.items():
            if call in abnormal_calls and frequency > threshold:
                print(f"비정상적인 시스템 호출 탐지: {call}")
                # 추가 분석 또는 경고 로직

    def perform_additional_dynamic_analysis(mobSF, file_hash):
        """각 Activity에 대한 추가 동적 분석 수행"""
        # 네트워크 트래픽 분석
        analyze_network_traffic(mobSF, file_hash)

        # 파일 시스템 변경 모니터링
        monitor_file_system_changes(mobSF, file_hash)

        # 비정상적인 시스템 호출 감지
        detect_abnormal_system_calls(mobSF, file_hash)        
...
if __name__ == "__main__":
    ...
    keystore_path = 'C:\\Users\\HP\\Desktop\\sandbox\\jobjo.keystore'  # Keystore 파일 경로
